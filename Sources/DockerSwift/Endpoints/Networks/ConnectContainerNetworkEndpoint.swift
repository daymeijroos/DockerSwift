...
public struct ConnectContainerNetworkEndpoint: SimpleEndpoint {
	...
	var body: ConnectSettings?

	private let networkNameOrId: String
	
	init(networkNameOrId: String, connectConfig: ConnectSettings) {
...
public extension ConnectContainerNetworkEndpoint {
	...
	struct ConnectSettings: Codable {
		...
		public struct EndpointSettings: Codable {
			public init(
				ipamConfig: ConnectSettings.EndpointSettings.IPAMConfig,
				links: [String]? = [],
				aliases: [String]? = [],
				networkId: String,
				...
				globalIPv6PrefixLen: UInt64? = nil,
